...
proc template;
	define statgraph SASStudio.Pie;
		begingraph;
		entrytitle 
			"Percentage of People With/Out Student Loans in America, By Generation (2023)" 
			/ textattrs=(size=14);
		entryfootnote halign=left 
			"0 = Do Not Have Student Loans, 1 = Have Student Loans" / 
			textattrs=(size=12);
		layout region;
		piechart category=GEN / group=STUDENTLOANS_BIN groupgap=2% stat=pct start=90 
			datalabellocation=inside dataskin=gloss;
		endlayout;
		endgraph;
	end;
run;
